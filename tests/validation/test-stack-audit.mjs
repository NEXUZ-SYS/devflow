...
test("T5 FAIL when manual-generation marker present (HTML comment)", () => {
  const { root, cleanup } = fixture();
  try {
    writeManifest(root, {
      foo: { version: "1.0.0", artisanalRef: "refs/foo@1.0.0.md" },
...
test("T5 PASS when prose mentions 'manually created' in legitimate context", () => {
  ...
  // The previous over-broad /manually created/i regex flagged it as a
  // marker. The refined pattern requires comment delimiters or standalone
  // disclaimer lines so prose doesn't trigger.
  const { root, cleanup } = fixture();
  try {
    writeManifest(root, {
      foo: { version: "1.0.0", artisanalRef: "refs/foo@1.0.0.md" },
    });
    const ref = VALID_REF.replace(
      "Snippet 1",
      "Snippet 1\n\nManually created type variables may be explicitly marked covariant or contravariant."
    );
    writeRef(root, "foo", "1.0.0", ref);
    const r = auditStack("foo", "1.0.0", root);
    const t5 = r.checks.find(c => c.id === "T5");
    assert.equal(t5.status, "PASS", "prose 'manually created' must NOT trigger T5");
  } finally {
    cleanup();
  }
});
