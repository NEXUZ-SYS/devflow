...
- **Stack:** Claude
- **Versao fixada:** Sonnet 4.6 / Opus 4.6
- **Camada arquitetural alvo:** Camada 2 — BFF
- **Papel nesta camada:** Provider PT-BR e reasoning
- **Categoria:** arquitetura
- **Slug sugerido:** adr-llm-claude-bff

## Contexto tecnico da Camada 2 (BFF)

Route handlers Next (app/api/**) feature-based. Orquestra chamadas aos servicos FastAPI, integra com LLMs via Mastra Model Router, consome MCP servers, gerencia auth de sessao via Firebase Admin SDK. Hospedado junto com o frontend em Firebase App Hosting.

**Stack desta camada:** Next route handlers + Vercel AI SDK v6 + Mastra Model Router (LLM-agnostic) + Mastra Workflows + Mastra Memory + MCP clients + NXZ Harness (pacote interno de orquestracao) + Gemini/Claude/OpenAI via router + Firestore Admin SDK + BigQuery client Node v8.
...
- `description: Claude Sonnet 4.6 / Opus 4.6 como Provider PT-BR e reasoning na camada BFF`
- `stack: Claude`
- `category: arquitetura` (`Arquitetura`)
...
- **Filename:** `<NNN>-adr-llm-claude-bff-v1.0.0.md` onde `<NNN>` e o proximo numero 3-digitos zero-padded.
