...
- **Versao fixada:** 9.x / 15.x
- **Camada arquitetural alvo:** Camada 1 — Frontend
- **Papel nesta camada:** Git hooks pre-commit
- **Categoria:** principios-codigo
- **Slug sugerido:** adr-husky-lint-staged-frontend

## Contexto tecnico da Camada 1 (Frontend)

App Next unico (web shell) com route groups, shell Tauri para builds desktop, Feature-Sliced Design + Atomic Design, Zustand para estado, Tailwind + shadcn/ui. Consome o BFF por route handlers.

**Stack desta camada:** Next.js 16 + React 19 + TypeScript 5.9 + Tailwind 4 + shadcn/ui + Tauri 2 + Zustand 5 + Vercel AI SDK v6 (AG-UI) + Vitest + Testing Library + Playwright + Storybook + MSW.
...
- `description: husky + lint-staged 9.x / 15.x como Git hooks pre-commit na camada Frontend`
- `stack: husky + lint-staged`
- `category: principios-codigo` (`Principios de Codigo`)
...
- **Filename:** `<NNN>-adr-husky-lint-staged-frontend-v1.0.0.md` onde `<NNN>` e o proximo numero 3-digitos zero-padded.
