...
3. Para enforcement no fluxo de código, prefira os linters do DevFlow (automáticos) + `/devflow:devflow-design audit`/`critique` (guidance). A extensão é para inspeção visual ad-hoc no navegador.

## Nota

O modo `live` (`/devflow:devflow-design live`) é coisa diferente da extensão: é iteração ao vivo no dev server via bridge (`scripts/design/live-bridge.mjs`), não um overlay de página. Ver `live.md`.
