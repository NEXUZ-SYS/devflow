...
| `/devflow:devflow-status` | Show current phase, progress, and available actions |
| `/devflow:devflow-next` | Attempt to advance to next phase (checks gates) |
| `/devflow:devflow-dispatch` | List available agents for current phase and mode |
| `/devflow:devflow-dispatch <role>` | Dispatch a specific agent |
| `/devflow:devflow-sync [scope]` | Update .context/ with current project state (docs/agents/skills) |
...
| `/devflow:devflow-sync workflow` | Validate/scaffold `.context/workflow/` directory |
